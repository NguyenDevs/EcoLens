...
class ButtonHandler(
    ...
    private fun shareSpeciesInfo(info: SpeciesInfo, imageUri: Uri?) {
        ...
        val shareText = buildString {
            ...
            append("\n━━━━━━━━━━━━━━━━━━━━\n\n")
            append("📌 ${textFormatter.stripHtml(info.commonName)}\n🔬 ${textFormatter.stripHtml(info.scientificName)}\n")
            ...
            if (info.kingdom.isNotEmpty()) append("• ${context.getString(R.string.label_kingdom)} ${textFormatter.stripHtml(info.kingdom)}\n")
            if (info.phylum.isNotEmpty()) append("• ${context.getString(R.string.label_phylum)} ${textFormatter.stripHtml(info.phylum)}\n")
            if (info.className.isNotEmpty()) append("• ${context.getString(R.string.label_class)} ${textFormatter.stripHtml(info.className)}\n")
            if (info.taxorder.isNotEmpty()) append("• ${context.getString(R.string.label_order)} ${textFormatter.stripHtml(info.taxorder)}\n")
            if (info.family.isNotEmpty()) append("• ${context.getString(R.string.label_family)} ${textFormatter.stripHtml(info.family)}\n")
            if (info.genus.isNotEmpty()) append("• ${context.getString(R.string.label_genus)} ${textFormatter.stripHtml(info.genus)}\n")
            if (info.species.isNotEmpty()) append("• ${context.getString(R.string.label_species)} ${textFormatter.stripHtml(info.species)}\n")
        ...
        try {
            val intent = Intent(Intent.ACTION_SEND).apply {
                ...
                putExtra(Intent.EXTRA_SUBJECT, context.getString(R.string.share_subject, textFormatter.stripHtml(info.commonName)))
            }
